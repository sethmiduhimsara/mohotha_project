...
import { Gemunu_Libre, Noto_Sans_Sinhala, Yaldevi } from "next/font/google";
import {
  Heart,
  MapPin,
  Navigation,
...
// Noto Sans Sinhala — clean, highly legible body font
            ...
            {timeline.map((item, i) => (
              <FadeIn key={item.num} direction="up" delay={i * 0.2}>
                <div className="relative flex flex-col gap-6 md:flex-row md:items-center">
                  {/* Left Seal Badge */}
                  <motion.div
                    className="relative z-10 flex h-14 w-14 flex-shrink-0 -translate-x-[27px] items-center justify-center rounded-full border-2 border-[#e8930a] bg-[#2d0a1a] shadow-[0_0_20px_rgba(232,147,10,0.3)] md:-translate-x-[35px]"
        ...
        >
          {images.map((img, i) => (
            <motion.div
              ...
              className={`group relative flex-shrink-0 cursor-pointer overflow-hidden rounded-3xl border-2 border-[#e8930a]/30 bg-[#2d0a1a] snap-center shadow-[0_20px_50px_rgba(45,10,26,0.12)] transition-all duration-500 hover:border-[#e8930a] hover:shadow-[0_30px_60px_rgba(232,147,10,0.3)] ${img.aspect}`}
...
function DressCodeSection() {
  return (
    <Section id="dress-code" className="relative overflow-hidden bg-[#2d0a1a] py-24 md:py-36">
      <LiyawelaPattern mode="dark" opacity={0.25} />

      {/* Ambient Lighting */}
      <div className="pointer-events-none absolute left-1/2 top-1/2 h-[600px] w-[600px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#e8930a]/10 blur-[160px]" />
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-8 md:px-16">
        <div className="flex flex-col items-center gap-16 md:flex-row">
          <FadeIn direction="right" className="w-full md:w-1/2">
            ...
            <p className={`${noto.className} mb-6 text-base font-normal leading-relaxed text-[#fdfaf4]`}>
            ...
            <p className={`${noto.className} mb-8 text-base font-normal leading-relaxed text-[#c4a882]`}>
              මනාලියගේ ඇඳුම් වර්ණය වන සුදු සහ ඇත්දළ (Ivory) පැහැයෙන් යුතු ඇඳුම් ඇඳීමෙන් වළකින මෙන් කාරුණිකව ඉල්ලා සිටිමු.
            </p>

            {/* Palette Swatches */}
            <div className="mb-8 rounded-2xl border border-[#e8930a]/40 bg-white/5 p-6 backdrop-blur-md shadow-xl">
              ...
              <div className="flex flex-wrap items-center gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`${noto.className} text-xs font-medium text-[#fef3e2]`}>Maroon</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="h-7 w-7 rounded-full bg-[#e8930a] shadow-md" />
                  <span className={`${noto.className} text-xs font-medium text-[#fef3e2]`}>Saffron Gold</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="h-7 w-7 rounded-full bg-[#7a4a2a] border border-[#e8930a]/40 shadow-md" />
                  <span className={`${noto.className} text-xs font-medium text-[#fef3e2]`}>Warm Bronze</span>
                </div>
          ...
          <FadeIn direction="left" className="w-full md:w-1/2">
            <motion.div
              className="relative h-[520px] overflow-hidden rounded-3xl border-2 border-[#e8930a]/40 shadow-[0_30px_80px_rgba(0,0,0,0.5)]"
            ...
            >
              ...
              />
              <div className="absolute inset-0 bg-[#2d0a1a]/20" />
              {/* Corner filigree accents */}
              <div className="pointer-events-none absolute left-6 top-6 h-8 w-8 border-l-2 border-t-2 border-[#e8930a]/80" />
              <div className="pointer-events-none absolute right-6 bottom-6 h-8 w-8 border-r-2 border-b-2 border-[#e8930a]/80" />
            </motion.div>
          </FadeIn>
        </div>
      </div>
